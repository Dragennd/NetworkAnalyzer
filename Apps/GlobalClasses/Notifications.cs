...
namespace NetworkAnalyzer.Apps.GlobalClasses
{
    ...
    {
        public static string ProcessResponseCodes(ResponseCode responsecode)
        {
            switch (responsecode)
            {
                case ResponseCode.Empty_Input_Exception:
                    return "Entry cannot be blank.";

                case ResponseCode.Input_Less_Than_Starting_Port_Exception:
                    return "Ending Port cannot be less than the Starting Port.";

                case ResponseCode.Invalid_IP_Address_Exception:
                    return "One or more IP Addresses are incorrectly formatted or a DNS name cannot be resolved.";

                case ResponseCode.Empty_Data_Collection_Exception:
