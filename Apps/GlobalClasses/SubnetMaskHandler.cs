...
namespace NetworkAnalyzer.Apps.GlobalClasses
{
    public class SubnetMaskHandler
    {
        ...
        // and filter out all IP Addresses and their associated Subnet Masks which are IPv6, APIPA or LinkLocal
        public static async Task<(List<string>, List<string>)> GetActiveNetworkInterfacesAsync()
        {
            ...
            var addresses =
                interfaceAddresses
                    ...
                    .Select(a => (a.Address.ToString(), a.IPv4Mask.ToString()))
                    .ToList();

            return (addresses.Select(x => x.Item1).ToList(), addresses.Select(x => x.Item2).ToList());
        }
}
